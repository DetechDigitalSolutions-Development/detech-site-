...
@endif
    ...
    >
    <img
        src="{{ Storage::disk(config('public'))->url($product->featured_img) }}"
        alt="product image"
        width="645"
        height="690"
        loading="lazy"
    ...
    <div class="card-project-content-absolute">
        ...
        <h2 class="heading text-24">{{ $product->product_title}}</h2>
        <p class="text text-16">{{ $product->short_description ?? null }}</p>
        </div>
    </div>
