...
<div class="min-h-screen">
            ...
            <div class="flex flex-col md:flex-row justify-between items-center gap-4">
                ...
                <div class="w-full md:w-auto">
                    <div class="relative">
                        <input 
                            type="text" 
                            placeholder="Search roles (e.g., 'Virtual Assistant', 'Marketing')..." 
                            class="w-full md:w-80 px-4 py-3 pl-12 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#205781] focus:border-transparent transition-all"
    ...
    <main class="py-8">
        <div class="container mx-auto px-4 md:px-8">
                ...
                <table class="min-w-full">
                    <thead>
                        <tr class="bg-gradient-to-r from-[#205781] to-[#4F959D] text-white">
                            <th class="py-6 px-6 text-left text-lg font-bold border-r border-white/20 sticky left-0 bg-[#205781] z-10 min-w-[250px]">
                                <div class="flex items-center">
                            ...
                            @foreach($currencies as $currency)
                                <th 
                                    class="py-6 px-6 text-left text-lg font-bold border-r border-white/20 min-w-[180px] currency-column currency-{{ $currency->code }}"
                                ...
                                >
                                    <div class="flex items-center justify-between">
                                        ...
                                        <!-- Toggle indicator -->
                                        <div class="w-3 h-3 rounded-full bg-green-400 ml-2 toggle-indicator-{{ $currency->code }}"></div>
                                    </div>
                                </th>
                            @endforeach
                        </tr>
                    ...
                    <tbody>
                        @foreach($salaryData as $guide)
                            ...
                            <tr 
                                class="transition-all duration-200 hover:bg-[#F6F8D5]/50"
                                data-role="{{ strtolower($guide['role']) }}"
                            >
                                <!-- Role and Experience Column -->
                                @if($index === 0)
                                <td class="p-6 border-t border-gray-100 align-top sticky left-0 bg-white z-10 min-w-[250px] border-r" rowspan="{{ count($guide['rates']) }}">
                                    <div class="flex flex-col h-full justify-center">
                                        <h3 class="text-xl font-bold text-[#205781] mb-2">{{ $guide['role'] }}</h3>
                                        <div class="space-y-2">
                                            @foreach($guide['rates'] as $subRate)
                                            <div class="flex items-center py-1">
                                                <div class="w-2 h-2 rounded-full bg-[#98D2C0] mr-3"></div>
                                                <div>
                                                    <span class="font-semibold text-gray-800">{{ $subRate['level'] }}</span>
                                                    <span class="text-sm text-gray-600 ml-2">{{ $subRate['description'] }}</span>
                                                </div>
                                            </div>
                                            @endforeach
                                        </div>
                                ...
                                @foreach($currencies as $currency)
                                    <td 
                                        class="p-6 border-t border-gray-100 border-r currency-column currency-{{ $currency->code }}"
                                        data-currency="{{ $currency->code }}"
                                    >
                                        ...
                                        <div 
                                            ...
                                            title="Click to copy"
                                        >
                                            <div class="text-2xl font-bold text-[#4F959D] group-hover:text-[#205781] transition-colors">
                                        ...
                                        <div class="text-sm text-gray-600 mt-2">
                                            @ {{ number_format($currency->exchange_rate * 100, 4) }}% of PHP
                                        </div>
                                        @endif
                                    </td>
                                @endforeach
                    ...
                    <div class="p-6">
                        ...
                        <div class="mb-6 pb-6 border-b border-gray-100 last:border-b-0 last:mb-0 last:pb-0">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                ...
                                @foreach($currencies as $currency)
                                    @php $convRate = $rate['converted_rates'][$currency->code]; @endphp
                                    <div class="bg-gradient-to-br from-[#98D2C0]/10 to-[#98D2C0]/5 p-4 rounded-lg border border-gray-200">
                                        <div class="text-sm font-semibold text-gray-600 mb-1">{{ $currency->label }}</div>
                                        <div class="text-2xl font-bold text-[#4F959D]">
                                            {{ $convRate['symbol'] }}{{ $convRate['min'] }} - {{ $convRate['symbol'] }}{{ $convRate['max'] }}
                                        </div>
                                        <button 
                                            onclick="copyToClipboard('{{ $guide['role'] }} - {{ $rate['level'] }}: {{ $convRate['symbol'] }}{{ $convRate['min'] }} - {{ $convRate['symbol'] }}{{ $convRate['max'] }}')"
            ...
            <!-- Information Section -->
            <div class="mt-12 bg-gradient-to-r from-[#F6F8D5] to-[#98D2C0]/20 rounded-2xl p-8 border border-[#98D2C0]/30">
                ...
                <div class="mt-8 pt-6 border-t border-gray-300">
                    <p class="text-sm text-gray-600">
                        <strong>Disclaimer:</strong> These salary ranges are for guidance only. Actual salaries may vary based on specific skills, company size, location, and other factors. Rates are for full-time positions (40 hours/week).
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="container mx-auto px-4 md:px-8">
            <div class="flex flex-col md:flex-row justify-between items-center">
                ...
                <div class="flex flex-wrap gap-4">
                    <a 
                        href="mailto:info@example.com?subject=Salary%20Guide%20Inquiry" 
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        window.toggleCurrency = function(currencyCode, isChecked) {
            ...
            if (isChecked) {
                ...
                document.querySelector(`.toggle-thumb-${currencyCode}`).style.transform = 'translateX(24px)';
                document.querySelector(`.toggle-label-${currencyCode}`).style.color = '#10b981';
                document.querySelector(`.toggle-indicator-${currencyCode}`).style.backgroundColor = '#10b981';
            } else {
                // Hide currency
                cells.forEach(cell => {
                ...
                document.querySelector(`.toggle-indicator-${currencyCode}`).style.backgroundColor = '#9ca3af';
            }
        };
