...
<main>
      <!-- Page Banner -->
      @include('components.page_banner',['title'=>'Our Projects','banner_image'=>'assets/img/banner/page-banner.png'])
            ...
            <nav class="pagination">
              ...
              >
                ...
                <li class="{{ $projects->onFirstPage() ? 'disabled' : '' }}">
                  <a
                    @if ($projects->previousPageUrl()) href="{{ $projects->previousPageUrl() }}" @else role="link" aria-disabled="true" @endif
                ...
                @foreach ($projects->getUrlRange(1, $projects->lastPage()) as $page => $url)
                  <li>
                    <a
                      href="{{ $url }}"
                      class="pagination-link {{ ($page == $projects->currentPage()) ? 'active' : '' }}"
                ...
                <li class="{{ $projects->hasMorePages() ? '' : 'disabled' }}">
                  <a
                    @if ($projects->nextPageUrl()) href="{{ $projects->nextPageUrl() }}" @else role="link" aria-disabled="true" @endif
                    class="pagination-link"
