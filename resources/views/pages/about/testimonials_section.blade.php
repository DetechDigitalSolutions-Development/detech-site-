                    ...
                    @for($i=0; $i < ($testimonial->rating ?? 5); $i++)
                      <li class="rating-icon">
                        <svg class="icon icon-24" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                          <path d="M11.9998 17L6.12197 20.5902L7.72007 13.8906L2.48926 9.40983L9.35479 8.85942L11.9998 2.5L14.6449 8.85942L21.5104 9.40983L16.2796 13.8906L17.8777 20.5902L11.9998 17Z" fill="CurrentColor"/>
                        </svg>
                      </li>
                    @endfor
                  </ul>
                  <p class="text text-24">“ {{ $testimonial->comments }} ”                    
                  </p>
                  <div class="user-info-wrap">
                    <div class="user-info">
                      <div class="user-img">
                        <img src="{{ Storage::disk(config('public'))->url( $testimonial->profile_img ) }}" width="80" height="80" loading="lazy" alt="image">
                      </div>
                      <div class="user-name-desig">
                        <h2 class="user-name heading text-24">{{ $testimonial->name }}</h2>
                        <div class="user-desig text text-18">{{ $testimonial->designation}}</div>
                      </div>
                    </div>
