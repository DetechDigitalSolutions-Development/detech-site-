...
@section('content')
    <main>
      <!-- Page Banner -->
      @include('components.page_banner',['title'=>'Our Blogs','banner_image'=>'assets/img/banner/page-banner.png'])
            ...
            <nav class="pagination">
              <ul class="list-unstyled pagintaion-list" data-aos="fade-up">
                ...
                <li class="{{ $blogs->onFirstPage() ? 'disabled' : '' }}">
                  <a
                    @if ($blogs->previousPageUrl()) href="{{ $blogs->previousPageUrl() }}" @else role="link" aria-disabled="true" @endif
                ...
                @foreach ($blogs->getUrlRange(1, $blogs->lastPage()) as $page => $url)
                  <li>
                    <a 
                      href="{{ $url }}"
                      class="pagination-link {{ ($page == $blogs->currentPage()) ? 'active' : '' }}"
                ...
                <li class="{{ $blogs->hasMorePages() ? '' : 'disabled' }}">
                  <a
                    @if ($blogs->nextPageUrl()) href="{{ $blogs->nextPageUrl() }}" @else role="link" aria-disabled="true" @endif
                    class="pagination-link"
