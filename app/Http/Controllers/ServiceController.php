...
class ServiceController extends Controller
{
    ...
    public function show($id)
    {
        ...
        // Set the first subservice as default active
        if (isset($service['subservices']) && count($service['subservices']) > 0) {
            $service['active_subservice'] = $service['subservices'][0];
        }
    ...
    /**
     * Get subservice content via AJAX
     */
    public function getSubservice($serviceId, $subserviceId)
    {
        $servicesData = $this->getServicesData();
        $services = $servicesData['services'] ?? [];
        ...
        $subservice = collect($service['subservices'])->firstWhere('id', $subserviceId);
